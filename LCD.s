...
poll    
    LDR R2, =SSI0_SR_R   ;Checks bit 4
	LDRB R1, [R2]         
	AND R1, #0x10        ;Isolate bit 4
 	CMP R1, #0x10        ;If bit 4 is equal to 1 (high) poll again
	BEQ poll
	
	LDR  R2, =DC         ;Clear PA6
    LDRB  R1, [R2]
	AND  R1, #0xBF       ;BF= 1011 1111
	STRB  R1, [R2]
	
	LDR  R2, =SSI0_DR_R  ;Writes the command to SSIO_DR_R	
    STRB R0, [R2]        ;R0 has data (given)

poll2
	LDR  R2, =SSI0_SR_R   ;Checks bit 4
	LDRB R1, [R2]                   
	AND  R1, #0x10        
	CMP  R1, #0x10        ;If bit 4 is equal to 1 (high) go to step 5
	BEQ poll2
...
loop    
    LDR  R2, =SSI0_SR_R   ;Checks bit 1
	LDRB R1, [R2]        
	AND  R1, #0x02        ;Isolate the bit
	CMP  R1, #0           ;If bit 1 is equal to 0 (low) poll again
	BEQ loop

    LDR   R2, =DC         ;Set PA6 to one
    LDRB  R1, [R2]
	ORR   R1, #0x40               ;0100 0000
	STRB  R1, [R2]
    
	LDR  R2, =SSI0_DR_R           ;Write data to SSIO_DR_R
	STRB R0, [R2]
