...
DataPt		SPACE 4		;pointer data var
DataBuffer 	SPACE 100	;array for data
TimePt		SPACE 4		;pointer time var
TimeBuffer 	SPACE 400	;array for time
...
;**********************
	  ...
	  ;place xFF in every entry of the data array
;**********************
...
bugloop		
		...
		ORR R1, #0xFF
        STRB R1, [R0]
		ADD R0, #1	   ;Increment spot in the array
		ADD R2, #1     ;R2 is couter
		CMP R2, #99   
		BLT bugloop
...
bugloop2		
		...
		ORR R1, #0xFFFFFFFF
        STR R1, [R0]
		ADD R0, #4     ;Increment spot in the array
		ADD R2, #1     ;R2 is couter
		CMP R2, #99   
		BLT bugloop2
...
Debug_Capture 
      PUSH {R0-R6,LR}
		LDR R1,= DataBuffer		;checks for full init data array
		ADD R1, #99
		LDR R0, [R1]
		CMP R0, #0xFF
		BNE immret
		LDR R1,= TimeBuffer		;checks for full init time array
		ADD R1, #399
		LDR R0, [R1]
		CMP R0, #0xFFFFFFFF
		BNE immret
		
		LDR R1,= GPIO_PORTA_DATA_R	;Read Ports A and E and do masking
		LDRB R0, [R1]
		AND R0, #0x10
		MOV R2,R0                   ;R2 has the state of PA4
		LDR R1,= GPIO_PORTE_DATA_R
		LDRB R0, [R1]
		AND R0, #0x0F               ;R0 has the state of PE3-PE0
		ORR R2,R2,R0                ;Combine into R2, states of PA4, PE2-PE0
		LDR R0,= DataPt 
		STRB R2, [R0]               ;Store the combine data into DataPt
		ADD R0, #1					;Increment data pointer
